...
public class Nuke extends Sprite{
    ...
    public void move() {
        ...
        if (getExpiry() % (EXPIRE/6) == 0) {
            nukeState++;
            // Additional effects for explosion and implosion
            if (nukeState <= 3) {
                // Explosion phase
                setRadius(getRadius() + 20); // Increased expansion
                // Color change to signify intensity
                setColor(new Color(255, (int)(Math.random() * 155) + 100, 0)); // Random shades of orange/red
                // Particle effects can be added here
            } else {
                // Implosion phase
                setRadius(getRadius() - 22);
                // Gradually fade color
                setColor(getColor().darker());
                // Additional implosion effects
            }
        }
//        super.move();
